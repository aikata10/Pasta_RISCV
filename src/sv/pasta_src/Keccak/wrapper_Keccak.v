...
module wrapper_Keccak(clk, rst, rounds_done,mode_type, inputByteLen, outputByteLen,
                      read_address, din_64bit,  
                      write_address, dout_64bit, we,
                      done
                    );
input clk;
input rst,rounds_done;  // Active high reset 
input [1:0] mode_type;      // 0->rate576, SHA512
							// 1->rate1088, SHA256
							// 2->rate1344, SHAKE128
...
assign rateInBytes = (mode_type==2'd0) ? 8'd72 :    // SHA_512 rate = 576/8 = 72 in bytes
                     (mode_type==2'd1) ? 8'd136 :   // SHA_256 rate = 1088/8 = 136 in bytes
                     (mode_type==2'd2) ? 8'd168 :    // SHAKE_128 rate = 1344/8 = 168 in bytes
                     8'd0;
                        
assign delimitedSuffix = (mode_type==2'd0) ? 8'h06 :    // SHA_512 uses delimitedSuffix = 0x06
                         (mode_type==2'd1) ? 8'h06 :    // SHA_256 uses delimitedSuffix = 0x06
                         (mode_type==2'd2) ? 8'h1f :    // SHAKE_128 uses delimitedSuffix = 0x1f
                         8'h00;
                         
assign inputLen_InBytes = inputByteLen;
assign outputLen_InBytes =  (mode_type==2'd0) ? 16'd64 :  // Output length for SHA512 is fixed to 512 bits
                            (mode_type==2'd1) ? 16'd32 : // Output length for SHA256 is fixed to 512 bits
                            (mode_type==2'd2) ? outputByteLen[15:0] : // Output length for SHAKE128 can be of any length. Here it is assumed to be a maximum of 2^16 bytes.
                            16'd0;

assign we = !rst && dout_valid;
...
assign din_64bit_to_keccak = (use_latched_din) ? din_64bit_latched : din_64bit ;                            

keccak_speed    KeccakCore(
                clk, rst_keccak, rounds_done,rateInBytes, delimitedSuffix, 
...
always @(state or keccak_is_ready_to_receive or keccak_notready_to_ready_transition)
begin
    case(state)
    4'd0: begin // Reset state
            rst_keccak<=1; keccak_squeeze_resume<=0; use_latched_din<=0;
            inc_read_address<=0; din_valid<=0;
        end

    4'd1: begin // Init Absorb state; Wait here untill keccak_is_ready_to_receive becomes 1.
            rst_keccak<=0; keccak_squeeze_resume<=0; use_latched_din<=0;
            inc_read_address<=0; din_valid<=0;
        end
    4'd2: begin // Absorb state; Fetch data from BRAM in 64 bit words. 
            rst_keccak<=0; keccak_squeeze_resume<=0; use_latched_din<=0;
            inc_read_address<=1; din_valid<=0;
        end
    4'd3: begin // Absorb state; Fetch data from BRAM in 64 bit words. Always check if keccak_is_ready_to_receive is 1.
            rst_keccak<=0; keccak_squeeze_resume<=0; 
            if(keccak_is_ready_to_receive) begin inc_read_address<=1; din_valid<=1; end
            else begin inc_read_address<=0; din_valid<=0; end 
            if(keccak_notready_to_ready_transition) use_latched_din<=1; else use_latched_din<=0;   
        end
        /* Example:
    ...
    4'd4: begin // Squeeze 
            rst_keccak<=0; keccak_squeeze_resume<=1; use_latched_din<=0;
            inc_read_address<=0; din_valid<=0;
        end
    4'd5: begin // End 
            rst_keccak<=1; keccak_squeeze_resume<=0; use_latched_din<=0;
    ...
    default: begin // Reset state
            rst_keccak<=1; keccak_squeeze_resume<=0; use_latched_din<=0;
            inc_read_address<=0; din_valid<=0; inc_write_address<=0;    
        end        
    endcase
end

always @(state or keccak_is_ready_to_receive or keccak_absorb_done or keccak_squeeze_done)
begin
    case(state)
    4'd0: nextstate <= 4'd1;
    4'd1: begin
